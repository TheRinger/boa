...
xx
	...
	int opt = 1;
	int ret1 = setsockopt(server_s, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
    int ret2 = bind(server_s, (struct sockaddr *) &server_sockaddr, sizeof (server_sockaddr));
	//printf("ret1: %d, ret2: %d, errno: %d\n", ret1, ret2, errno);
	return ret1 | ret2;
}
